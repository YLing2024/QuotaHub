...
// 安全策略保持不变: vm realm 隔离(不注入任何宿主对象) + 黑白名单纵深防御 + 脚本超时 + SSRF 防护

export const TIMEOUT_MS = 15000

// 沙箱脚本执行时长上限(config 中已按 100~30000ms 钳制, 默认 2000)
export const EXTRACT_TIMEOUT_MS = config.scriptTimeoutMs

// 默认禁止访问环回/内网/云元数据地址(SSRF); 需要监控内网平台时设 QUOTAHUB_ALLOW_PRIVATE=1
const ALLOW_PRIVATE = config.allowPrivate
...
function decodeUnicodeEscapes(code: string): string {
  return code.replace(/\\u([0-9a-fA-F]{4})/g, (_m, hex: string) => String.fromCharCode(parseInt(hex, 16)))
}
...
function checkTopLevelWhitelist(code: string): void {
  const stripped = code
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    .replace(/\/\/[^\n\r]*/g, ' ')
    .replace(/'(?:\\.|[^'\\\n])*'|"(?:\\.|[^"\\\n])*"/g, '""')
...
export function resolvePath(obj: unknown, pathExpr: unknown): unknown {
...
export function evalJsResponse(text: unknown): unknown {
  ...
  try {
    ...
    if ((e as NodeJS.ErrnoException).code === 'ERR_SCRIPT_EXECUTION_TIMEOUT') {
      throw new Error('响应求值超时', { cause: e })
    }
    throw new Error(`响应不是有效 JSON/JS: ${(e as Error).message}`, { cause: e })
  }
}
...
export function runParse(raw: unknown, src: unknown): unknown {
  const code = String(src ?? '').trim()
  if (!code) throw new Error('未配置解析函数')
  // raw 是原始字符串(原始值, 无 constructor 链), 放入沙箱全局供解析代码引用
  const sandbox = Object.assign(Object.create(null) as Record<string, unknown>, { raw: String(raw) })
  vm.createContext(sandbox)
  try {
    return vm.runInNewContext(`(${code})(raw)`, sandbox, { timeout: EXTRACT_TIMEOUT_MS })
  } catch (e) {
    if ((e as NodeJS.ErrnoException).code === 'ERR_SCRIPT_EXECUTION_TIMEOUT') {
      throw new Error(`解析函数执行超时 (${EXTRACT_TIMEOUT_MS}ms)`, { cause: e })
    }
    throw new Error(`解析函数执行失败: ${(e as Error).message}`, { cause: e })
  }
}
...
export function runExtractor(src: unknown, data: unknown): unknown {
  ...
  try {
    ...
    if ((e as NodeJS.ErrnoException).code === 'ERR_SCRIPT_EXECUTION_TIMEOUT') {
      throw new Error(`提取函数执行超时 (${EXTRACT_TIMEOUT_MS}ms)`, { cause: e })
    }
    throw new Error(`提取函数执行失败: ${(e as Error).message}`, { cause: e })
  }
}

// 抓取目标: Platform 或 validate 接口的任意松散配置
export interface FetchTarget {
  request?: Partial<PlatformRequest>
  handler?: string
  extractor?: string
  parse?: string
...
export async function fetchBalance(platform: FetchTarget): Promise<{ value: number | string }> {
  ...
  try {
    ...
    throw new Error(
      (err as Error).name === 'AbortError' ? `请求超时 (${TIMEOUT_MS}ms)` : `请求失败: ${(err as Error).message}`,
      { cause: err },
    )
  } finally {
    clearTimeout(timer)
